...
4. Combining multiple plots into one image using `fact_wrap()`
...
test_that("test limma + voom work + work together", {
  ...
  # limma
  limma_res <- run_limma(counts_df, design, group, voom=FALSE)
  expect_equal(dim(limma_res), c(15026, 6))
  expect_equal(names(limma_res), c("day4", "day4vsday7", "AveExpr", "F", 
                                   "P.Value", "adj.P.Val"))
  # voom
  dge <- DGEList(counts=counts_df, group=group)
  expect_warning(voom_res <- run_voom(dge, design)) # voom yelling, ignore
...
```{r eval=FALSE}
  ...
  ...
```
Another simple set of tests, this time we want to ensure there are 3,000 rows and _3_ columns. We also want to ensure there aren't more than 1,000 rows of any package type.
