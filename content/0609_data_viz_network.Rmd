...
To assign color to edges, use `vertex_attr()`. `gsize()` returns the total number of edges. 
```{r}
...
To visualize the network, there's just... one simple function!  
```{r}
plot(samp_g)
```

But, in real-world, we don't want a random network. Now let's create our network from scratch.  
...
1. Create 2 columns in the data frame: `from` and `to`. Each row represents an edge, and the entries are vertex names. For instance, as indicated in row 1, there is an edge "from Crystal to Moonstone". In an undirected graph, you can put the two vertices in either way.  
