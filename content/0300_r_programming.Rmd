...
covered in [greater detail later](#factors)
...
[`factor()` function](https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/factor):
...
The factor `case_status` prints as a vector of labels, either `Disease` or
...
Note the order of the factor levels has changed so that controls, which have a
value of 1, precede disease, which have a value of 2. The integers assigned to
each level can be specified explicitly when creating the factor:
...
Controlling the order of levels in a factor is important in a number of
situations. One is when specifying the reference category for categorical
variables when constructing model matrices to pass to statistical models, the
details of which are beyond the scope of this book. A second is when the order
of categorical variables when passed to ggplot, which is
covered in greater detail in [Reordering 1-D Data Elements] in the
[Grammar of Graphics] chapter. The [forcats](https://forcats.tidyverse.org/)
tidyverse package provides more powerful functions for working with categorical
variables stored in factors.
...
::: {.box .note}
R can store hyper-rectangular data using `array`s. `arrays` are constructed and
operate similarly to matrix, but may have arbitrarily many dimensions. The
following example is a 3-dimensional array with dimension sizes of $2x2x3$:
...
Each of the data structures in R are collections of objects. Very often, we
would like to select a subset of elements in a collection that have certain
properties, for instance numeric values less than a specific threshold or occupy
a specific position in the vector (e.g. the first or last element). The process
of selecting out a subset of elements in a data structure is called *subsetting*.
R provides many different methods to subset a data structure, where the choice
of method depends on the type of data structure (e.g. vector vis a vis a data
frame) or the type of subsetting required by a particular use case.

::: {.box .warning}
Unlike other languages like C and python, R uses *1-based indexing*. This means
that the first item in any data structure is referenced with the index 1:
...
C and python use *0-based indexing*, where the first item in a data structure is
referenced with the index 0. For example:
...
As you move between developing code in 0-based languages and R, be sure to
remember they use different indexing strategies!
:::
...
To see the other methods of indexing vectors, see the section on [subsetting
vectors](https://adv-r.hadley.nz/subsetting.html#atomic-vectors-1) and Hadley
Wickham's book [Advanced R](https://adv-r.hadley.nz/).

#### Matrices

Since matrices in R are vectors, they may be subset using the same methods as
vectors. Additionally, because matrices also have a notion of rows and columns,
they may also be subset by pairs of vectors that select either rows or columns.
A matrix `x` may be subset with the syntax `[<row selectors>, <column selectors>]`.
The `selectors` may be any of the methods used to subset a vector, as described
above.
...
Note that when indexing a list with `[`, the result returned is always another
list; we will discuss this more later.

When a list has names associated with its values, it may also be access by name
...
We will discuss what `$` means at the end of this section.

#### Data frames

Data frames may be accessed like both vectors (and therefore lists) and also
like matrices, since they are rectangular by construction.


#### `[[` and `$`

The `[[` and `$` operators are used to access single items of data structures.
Remember from the section on subsetting lists that indexing with `[` (i.e. a
single square bracket) always returns a list. When we wish to access an
individual value of a list or data frame and return only that value, we use
`[[` when indexing by integer or name:
...
To see another (ane more complete) treatment of subsetting, see the section on
[subsetting vectors](https://adv-r.hadley.nz/subsetting.html#atomic-vectors-1)
and Hadley Wickham's book [Advanced R](https://adv-r.hadley.nz/).
...
Vectors, matrices, lists, and data frames can have names assigned to their
indices. For vectors and lists, these names are one-dimensional vectors of
characters that are assignable and accessible using the `names()` function:
...
When a list is created with named items, those names are accessible similarly:
...
As matrices and data frames also have both rows and columns, they can have
...
When performing logical tests on vectors, the `all()` function may be used to
return a single boolean value when all results are true:
...
Here, `arg1` and `arg2` are *arguments* or *actual arguments*, indicating
...
In the above example, `arg1` and `arg2` are *positional* arguments. This means
the order of arguments is important. Calling the function above like
`function_name(arg2, arg1)` will produce a different result than with
`function_name(arg1, arg2)`. As we will see shortly, functions have names for
their arguments in their definition that can be used to specify arguments in any
order.

For most functions, some arguments are *required*. This means that the function
will not execute without two arguments provided and will raise an error
...
arguments that are not explicitly listed in the signature. This syntax is called
...
these loop structures can have poor performance, and should generally be
...
While it is always a good idea to save results in tabular form in [CSV Files],
...
the [`saveRDS()` and `readRDS()`](https://stat.ethz.ch/R-manual/R-devel/library/base/html/readRDS.html)
functions:
...
These functions are very convenient for saving results of complicated analyses
and reading them back in later, especially if those analyses were time
consuming.

::: {.box .note}
R also has the functions
[`save()`](https://stat.ethz.ch/R-manual/R-devel/library/base/html/save.html)
and [`load()`](https://stat.ethz.ch/R-manual/R-devel/library/base/html/load.html).
These functions are similar to `saveRDS` and `readRDS`, except they do not allow
loading individual objects into new variables. Instead, `save` accepts one or
...
Then later when the file `saved_variables.rda` is `load()`ed, the all the
...
reasons, `saveRDS` and `loadRDS` are generally safer to use as you can be more
explicit about what you are saving and loading.
:::

::: {.box .readmore}
* [`saveRDS()` and `readRDS()` R manual](https://stat.ethz.ch/R-manual/R-devel/library/base/html/readRDS.html)
* [`readRDS()` and `saveRDS()` official documentation](https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/readRDS)
:::
...
Consistently formatted code is generally much easier to read than inconsistently
...
you install styler with `install.packages("styler")` in RStudio, a new entry is
available in the Addins menu:

![styler Addin menu](styler.png)

These Addins allow you to let styler format your code for you according to some
reasonable (albeit arbitrary) conventions.
